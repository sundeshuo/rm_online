 ...
 * @help
 * ===================================================================================
 * 加入本插件后，会增加一个在线模式，在线模式下回启用同屏/聊天/云变量功能。
 * 
 * 云变量请使用插件指令改变状态。
 * 
 * 后续会更新服务端配置，可以根据设置，定时改变云变量。
 * 
 * 使用云变量需要注意，云变量使用的依然是rm自带的开关变量独立开光，所以使用前
 * 清规划好哪些开关/变量分配给云变量，千万不要出现云变量还去使用rm原始事件改变
 * 开关/变量的状态的情况，使用rm原始事件改变状态并不会同步到服务器，会导致客户
 * 端之间开关/变量状态不一致的情况。
 * ===================================================================================
 * @command Sync Switche
 * @text 同步开关
 * @desc 此操作将改变在线模式所有玩家该开关状态
 * 
 * @arg Switche
 * @type switch
 * @text 开关
 ...
 * @default true
 * 
 * @command Sync Variable
 * @text 同步变量
 * @desc 此操作将改变在线模式所有玩家该变量的值
 * 
 * @arg Variable
 * @type variable
 * @text 变量
...
Imported.sun_online_expand = true;
var sun_online_expand_param = sun_online_expand_param || {};
sun_online_expand_param.parameters = PluginManager.parameters('sun_online_expand');
sun_online_expand_param.maxChat = Number(sun_online_expand_param.parameters['chat log max'] || 0);
sun_online_expand_param.map = Number(sun_online_expand_param.parameters['Online Map'] || 0);
sun_online_expand_param.mapX = Number(sun_online_expand_param.parameters['Online Map X'] || 0);
sun_online_expand_param.mapY = Number(sun_online_expand_param.parameters['Online Map Y'] || 0);


//====================第三方库socket.io ，用于与服务器双向通讯====================
...
const socket = io(sun_online_param.url);
//====================复写rm核心====================

const sun_commandNewGame = Scene_Title.prototype.commandNewGame;
Scene_Title.prototype.commandNewGame = function () {
    $mode = "local";
    sun_commandNewGame.call(this);
}

//增加在线模式
const sun_title_makeCommandLis = Window_TitleCommand.prototype.makeCommandList;
Window_TitleCommand.prototype.makeCommandList = function () {
    this.addCommand("在线模式", "online");
    sun_title_makeCommandLis.call(this);
}

const sun_title_createCommandWindow = Scene_Title.prototype.createCommandWindow;
Scene_Title.prototype.createCommandWindow = function () {
    sun_title_createCommandWindow.call(this);
    this._commandWindow.setHandler("online", this.commandOnline.bind(this));
};

//在线模式出生位置
const sun_setupForNewGame = Game_Player.prototype.setupForNewGame;
Game_Player.prototype.setupForNewGame = function () {
    if ($online._mode === "online") {
        const mapId = $dataSystem.startMapId;
        const x = sun_online_expand_param.mapX;
        const y = sun_online_expand_param.mapY;
        this.reserveTransfer(mapId, x, y, 2, 0);
        return;
    }
    sun_setupForNewGame.call(this);
}

Scene_Title.prototype.commandOnline = function () {
    $mode = "online";
    online.logon();
}

//读档时，如果是在线模式存档则通知服务器位置信息
const sun_onLoadSuccess = Scene_Load.prototype.onLoadSuccess;
Scene_Load.prototype.onLoadSuccess = function () {
    sun_onLoadSuccess.call(this);
    $chat = new chat();
    if ($online._mode === "online") {
...
Game_Switches.prototype.setNetValue = function (switchId, value) {
    if (switchId > 0 && switchId < $dataSystem.switches.length) {
        ...
        this.onChange();
        if ($online._mode === "online") {
            socket.emit('switches change', { "switchId": switchId, "value": value });
        }
    }
}
...
Game_Variables.prototype.setNetValue = function (variableId, value) {
    if (variableId > 0 && variableId < $dataSystem.variables.length) {
        ...
        this.onChange();
        if ($online._mode === "online") {
            socket.emit('variables change', { "variableId": variableId, "value": value });
        }
    }
}
...
Game_SelfSwitches.prototype.setNetValue = function (key, value) {
    ...
    this.onChange();
    if ($online._mode === "online") {
        socket.emit('selfSwitches change', { "key": key, "value": value });
    }
}


//====================socket事件====================
online.createSocketEvent = function () {

    //地图刷新增加刷新同屏其他玩家
    const sun_map_updateMain = Scene_Map.prototype.updateMain;
    Scene_Map.prototype.updateMain = function () {
        sun_map_updateMain.call(this);
        $online.other.update();
    }

    //====================socket事件====================
    //当有玩家传送
    socket.on('transfer player', (data) => {
        if (!$online.other._data[data.user]) {
            $online.other._data[data.user] = new online_Character(data.actorId);
            $online.other._spriteData[data.user] = new Sprite_Character($online.other._data[data.user]);
            $online.other._nameSprit[data.user] = new Sprite_OnlineName(data.user);
        }
        $online.other._data[data.user].locate(data.x, data.y);
        $online.other.add(data.user);
    })

    //场所移动后通知服务器
    const sun_performTransfer = Game_Player.prototype.performTransfer;
    Game_Player.prototype.performTransfer = function () {
        if (this._newMapId != $gameMap.mapId() && $online._isLogon === true) {
            socket.emit('player map leave', { "user": $online._name, "mapId": $gameMap.mapId() });
            socket.emit('get map player', this._newMapId);
        }
        sun_performTransfer.call(this);
    ...
    //接受当前地图玩家数据
    socket.on('map all player', (data) => {
        // console.log(data);
        $online.other.reset();
        for (const user of data) {
            if (user.user === $online._name) continue;
            $online.other._data[user.user] = new online_Character(user.actorId);
            $online.other._spriteData[user.user] = new Sprite_Character($online.other._data[user.user]);
            $online.other._nameSprit[user.user] = new Sprite_OnlineName(user.user);
            // console.log(player);
            $online.other._data[user.user].locate(user.x, user.y);
        }
        $online.other.addAll();
    })
    ...
    socket.on('player move', (data) => {
        const player = $online.other._data;
        if (!player[data.user]) return;
        player[data.user].move(data.x, data.y);

    })

    //向相同地图其他玩家广播移动信息
    const sun_executeMove = Game_Player.prototype.executeMove;
    Game_Player.prototype.executeMove = function (direction) {
        sun_executeMove.call(this, direction);
        if ($online._isLogon === true) {
            const data = {
                "user": $online._name,
                "mapId": $gameMap.mapId(),
    ...
    socket.on('player map leave', (data) => {
        ...
        $online.other.remove(user);
    })

    //其他客户端登录弹出
    socket.on('other client login', (data) => {
        online.tipWindowPopup("其他客户端登录，已执行自动保存，5s后将强制退出游戏！")
        Scene_Base.prototype.executeAutosave.call(this);
        setTimeout(() => {
    ...
    socket.on('switches change', (data) => {
        $gameSwitches.setValue(data.switchId, data.value);
    })
    socket.on('variables change', (data) => {
        $gameVariables.setValue(data.variableId, data.value);
    })
    socket.on('selfSwitches change', (data) => {
        $gameSelfSwitches.setValue(data.key, data.value);
    })

    //同步变量开关
    socket.on('sync switches', (data) => {
        const switches = data.switches;
        const variables = data.variables;
        const selfSwitches = Object.keys(data.selfSwitches);
        for (let [index, item] of switches) {
            $gameSwitches.setValue(index, item);
        }
        for (let [index, item] of variables) {
            $gameVariables.setValue(index, item);
        }
        for (let item of selfSwitches) {
            $gameSelfSwitches.setValue(item, data.selfSwitches[item]);
        }
    })
}
...
class chat {
    constructor() {
        ...
        this._chat.className = "chat";
        this._chat.innerHTML = '<div class="chat_main"><div class="more"><div class="top"><button class="chat_esc" onclick="$chat.close()">关闭</button></div></div></div><div class="chat_conten" id="chat"></div><div class="chat_input"><input type="text" class="i3" id="chatInput"><button class="chat_send" onclick="$chat.chatSend()">发送</button></div>';
        this._chat.addEventListener("touchstart", stopPropagation);
        this._chatConten = this._chat.getElementsByClassName("chat_conten")[0];
    ...
    createMessage(data) {
        ...
        if (data.name === $online._name) {
            message.className = "myself";
        }
        else {
    ...
    addMessage(data) {
        ...
        this._chatConten.appendChild(message);
        const max = sun_online_expand_param.maxChat;
        if (this._chatConten.children.length > max) {
            this._chatConten.children[0].remove();
        }
        //将滚动条滑到最下方
        this._chatConten.scrollTop = this._chatConten.scrollHeight;
    }
    ...
    chatSend() {
        ...
        const data = JSON.stringify({ "name": name, "text": value });
    ...
    addMessage(data) {
        const message = this.createMessage(data);
        const chat = document.getElementById("chat");
        // this._chatConten = document.getElementById("chat");
        this._chatConten.appendChild(message);
        this._chatConten.scrollTop = this._chatConten.scrollHeight;
    }
...
// //如果传送到新地图，获取该地图所有玩家
// const sun_clearTransferInfo = Game_Player.prototype.clearTransferInfo;
// Game_Player.prototype.clearTransferInfo = function () {
//     if (this._newMapId != $gameMap.mapId()) {
//         socket.emit('get map player', this._newMapId);
//         console.log(123);
//     }
//     sun_clearTransferInfo.call(this);
// };



// socket.on('map move', (msg) => {
//     const data = JSON.parse(msg);
//     $chat.addMessage(data);
// })

class online_Character extends Game_Character {
    initialize(actorId) {
...
class other {
    ...
    setup() {
        // this._data = [];
        // for (let i = 0; i < $online.other.length; i++) {
        //     this._data.push(new Game_Follower($online.other[i]));
        // }
    }
    ...
    add(user) {
        const map = SceneManager._scene._spriteset._tilemap;
        map.addChild(this._spriteData[user]);
        SceneManager._scene.addChild(this._nameSprit[user]);
    }

    addAll() {
        const map = SceneManager._scene._spriteset._tilemap;
        const data = Object.keys(this._spriteData);
        for (const player of data) {
            map.addChild(this._spriteData[player]);
            SceneManager._scene.addChild(this._nameSprit[player]);
        }
    }

    remove(user) {
        const map = SceneManager._scene._spriteset._tilemap;
        map.removeChild(this._spriteData[user]);
        SceneManager._scene.removeChild(this._nameSprit[user]);
        this._data[user] = null;
        this._spriteData[user] = null;
        this._nameSprit[user] = null;
    }

    refresh() {
        for (const other of this._data) {
            other.refresh();
        }
    }
    ...
    update() {
        ...
        //     this.updateMove();
        // }
        const data = Object.keys(this._data);
        for (const other of data) {
            if (this._data[other] === null) continue;
            this._data[other].update();
        }
    }
}

class Sprite_OnlineName extends Sprite {
    initialize(user) {
        super.initialize();
        this._user = $online.other._data[user];
        this.createNameSprite(user);//绘制名字
        this.refresh();	//刷新信息
    }

    createNameSprite(user) {
        this._nameSprite = new Sprite(new Bitmap(144, 50));
        this._nameSprite.anchor.x = 0.5;
        this._nameSprite.anchor.y = 0.5;
        this._nameSprite.x = this._user.screenX();
        this._nameSprite.y = this._user.screenY() - 50;
        this._nameSprite.bitmap.fontSize = 20;
        this._nameSprite.bitmap.drawText(user, 0, 0, this._nameSprite.width, this._nameSprite.bitmap.fontSize, 'center');
    ...
    refresh() {
        this._nameSprite.x = this._user.screenX();
        this._nameSprite.y = this._user.screenY() - 50;
        this._nameSprite.bitmap.textColor = '#ffff00';
    }

    update() {
        if (this._nameSprite.x !== this._user.screenX() || this._nameSprite.y !== this._user.screenY() - 50) {
            this.refresh();
        }
    }
}
...
//====================插件指令===================
PluginManager.registerCommand("sun_online_expand", "Sync Switche", function (args) {
    const switche = Number(args.Switche);
    const value = args.Value === "true";
    $gameSwitches.setValue(switche, value);

});
PluginManager.registerCommand("sun_online_expand", "Sync Variable", function (args) {
    const variable = Number(args.Variable);
    const value = Number(args.Value);
    $gameVariables.setValue(variable, value);
});
PluginManager.registerCommand("sun_online_expand", "Sync SelfSwitche", function (args) {
    const mapId = this._mapId;
    const eventId = this._eventId;
    const switche = args.SelfSwitche;
    const value = args.Value === "true";
    const key = `${mapId},${eventId},${switche}`;
